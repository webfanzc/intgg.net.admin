...
var droppacksRouter = express.Router();
...
var droppacksDao = require('../daos/droppacksDao');
    ...
    async.auto({
        getInfoByintid:function (callback) {
            ...
            if(!_.isEmpty(packname)){
                var regstr = new RegExp(packname,"gi");
                _.extend(condition,{packname:regstr});
            }
            if(!_.isEmpty(date)){
            ...
            droppacksDao.getList(condition, null,queryParams, function (err, result) {
...
//保存素材
droppacksRouter.post('/save',function (req, res, next) {
    var params = URL.parse(req.url, true);
    ...
    async.auto({
        ...
        saveMate: ["checkintid",function (result, callback) {
            ...
            droppacksDao.save(body,function(err,reply){
                if (err) {  //内部服务错误
                    status = 500;
                    return callback(err);
                }
...
//更新素材
droppacksRouter.post('/update',function (req, res, next) {
    var params = URL.parse(req.url, true);
    var droppackid = req.query._id;
    var body = req.body;
    ...
    body = _.omit(body,"createTime", "updateTime","_id");

    async.auto({
        saveToMongo: function (callback) {
            droppacksDao.update({_id: droppackid},body,null,function(err,reply){
                if (err) {  //内部服务错误
                    status = 500;
                    return callback(err);
                }
...
//删除素材
droppacksRouter.post('/del',function (req, res, next) {
    var params = URL.parse(req.url, true);
    var intid = req.query.intid,
        token = req.query.token,
    ...
    async.auto({
        ...
        checkintid: ["checktoken",function (result, callback) {
            droppacksDao.findById(materialid,function(err,reply){
                logger.info(err,reply);
                if (err) {  //内部服务错误
                    status = 500;
                    return callback(err);
        ...
        saveToMongo: ["checkintid",function (result, callback) {
            droppacksDao.del({_id:materialid},null,function(err,reply){
                if (err) {  //内部服务错误
                    status = 500;
                    return callback(err);
                }
...
module.exports = droppacksRouter;